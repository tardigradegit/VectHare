...
/**
 * Persists a single-field change from the settings panel's `settings` snapshot onto
 * extension_settings.vecthare, EXCLUDING `collections` and `vecthare_collection_registry`.
 *
 * `settings` is captured once when renderSettings() runs and every input handler mutates
 * one field on it, then historically did `Object.assign(extension_settings.vecthare,
 * settings)` to persist that one field - but that also overwrites `collections` and
 * `vecthare_collection_registry` with this stale snapshot. Both of those are mutated
 * directly on extension_settings.vecthare by other code paths after the snapshot was taken
 * (setCollectionMeta/setCollectionEnabled write into .collections; collection-loader.js's
 * registry helpers reassign .vecthare_collection_registry to a new array), so blindly
 * overwriting them here silently rolled back every collection pause/rename/registry update
 * made since the settings panel was opened, the instant the user changed any unrelated
 * global setting (VectHare audit ui-a finding #1 - CRITICAL data loss).
 * @param {object} settings The settings snapshot to merge
 */
function applySettingsSnapshot(settings) {
    const { collections, vecthare_collection_registry, ...safeSettings } = settings;
...
function handleDiagnosticFix(action, silent = false) {
    ...
    switch (action) {
        case 'enable_chats':
            // #vecthare_autosync_enabled's handler is bound with .on('input', ...)
            // (bindSettingsEvents, ~line 1624) - .trigger('change') never fired it, so
            // this "fix" silently did nothing while still reporting success.
            $('#vecthare_autosync_enabled').prop('checked', true).trigger('input');
        ...
        case 'fix_threshold':
            // Handler is bound with .on('input', ...) at ~line 1932, not 'change'.
            $('#vecthare_score_threshold').val(0.25).trigger('input');
            if (!silent) toastr.success('Score threshold reset to 0.25');
            break;

        case 'fix_counts':
            // #vecthare_insert is never rendered (there is no standalone "insert count"
            // input in the settings panel - settings.insert only exists as a legacy
            // fallback read via `settings.top_k ?? settings.insert` elsewhere), so there
            // is no live control to fix it through. #vecthare_topk is the real control for
            // that value; fixing it below covers the practical case.
            if ((settings.top_k ?? settings.insert) < 1) {
                // Handler is bound with .on('input', ...) at ~line 2101, not 'change'.
                $('#vecthare_topk').val(3).trigger('input');
            }
            if (settings.query < 1) {
                // The query-depth control's actual ID is #vecthare_query_depth, not
                // #vecthare_query (which doesn't exist); handler bound on 'input' at
                // ~line 2088, not 'change'.
                $('#vecthare_query_depth').val(2).trigger('input');
            }
